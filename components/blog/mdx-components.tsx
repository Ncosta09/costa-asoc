...
export const mdxComponents: MDXComponents = {
  ...
  table: (p) => (
    <div className="my-7 overflow-x-auto">
      <table className="w-full border-collapse text-left text-[15.5px]" {...p} />
    </div>
  ),
  thead: (p) => <thead className="border-b border-cream-300" {...p} />,
  th: (p) => (
    <th className="px-3 py-2.5 font-display text-[14px] font-semibold tracking-tight text-navy-900" {...p} />
  ),
  td: (p) => (
    <td className="border-b border-cream-200 px-3 py-2.5 align-top leading-relaxed text-ink-700" {...p} />
  ),
};
